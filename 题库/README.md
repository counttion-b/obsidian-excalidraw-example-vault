...
对于讲义或试卷中只有题干、题目来源需要重新整理的情况，可以运行：

```text
当前试卷：一键更新题目来源（北京优先）
```

它会用每道题的题干检索教研云，在匹配到的全部来源中优先选择有明确学校名的北京来源，再按学年最新、学校偏好顺序选择；结果会写入同目录的 `来源版.md`，原文件不变。北京学校的默认偏好顺序可以在插件设置中修改。如果题干匹配分过低，或匹配结果没有合格的北京学校来源，该题会保留题干并将标题标为“例题”。

运行“当前纯文字试卷：一键填充图片”时，成功匹配的题目也会同步使用筛选后的来源标题；已经有图片的题目如需重新筛选来源，可单独运行上面的来源更新命令。
